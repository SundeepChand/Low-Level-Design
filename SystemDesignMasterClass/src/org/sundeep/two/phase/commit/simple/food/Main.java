package org.sundeep.two.phase.commit.simple.food;

import com.sun.net.httpserver.HttpExchange;
import com.sun.net.httpserver.HttpServer;
import org.sundeep.two.phase.commit.simple.food.configs.ServerConfig;
import org.sundeep.two.phase.commit.simple.food.service.FoodService;
...
class ServerHandler {
    ...
    public void handlePrepareAgentAssignment(HttpExchange exchange) throws IOException {
        ...
        try {
            int foodId = service.reserveFoodForOrder(orderId);
            byte[] responseBody = Integer.toString(foodId).getBytes();
    ...
    public void handleCommitAgentAssignment(HttpExchange exchange) throws IOException {
        ...
        // Read orderId from query parameters (e.g. /agent/assign?orderId=123&foodId=456)
        String orderId = getQueryParam(exchange, "orderId");
        String agentId = getQueryParam(exchange, "foodId");
        ...
        try {
            service.assignFoodToOrder(orderId, agentId);
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // 1. To prepare a food for assignment
        // 2. To commit the order assignment to the food
        ServerHandler serverHandler = new ServerHandler();

        HttpServer server = HttpServer.create(new InetSocketAddress(ServerConfig.SERVER_PORT), 0);

        server.createContext("/food/prepare", serverHandler::handlePrepareAgentAssignment);

        server.createContext("/food/assign", serverHandler::handleCommitAgentAssignment);
